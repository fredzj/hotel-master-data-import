        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4>Edit Room</h4>
                    <div>
                        <a href="{{ route('rooms.show', $room) }}" class="btn btn-secondary me-2">
                    ...
                    <form method="POST" action="{{ route('rooms.update', $room) }}">
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control @error('number') is-invalid @enderror" 
                                           id="number" name="number" value="{{ old('number', $room->number) }}" required>
                                    @error('number')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Enter room description (optional)">{{ old('description', $room->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <p><strong>Hotel:</strong> {{ $room->roomType->hotel->name ?? 'N/A' }}</p>
                                        <p><strong>Current Room Type:</strong> {{ $room->roomType->name ?? 'N/A' }}</p>
                                    </div>
                                    <div class="col-md-6">
                                        <p><strong>External ID:</strong> {{ $room->external_id ?? 'N/A' }}</p>
                                        <p><strong>Attributes:</strong> {{ $room->attributes->count() ?? 0 }} attributes</p>
                                    </div>
                                </div>
                            </div>
                        </div>
